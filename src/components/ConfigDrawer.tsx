...
export const DEFAULT_WORDMARK = "${config.wordmark}";`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs">
      <div 
        className="bg-[#284871] border-l border-[#FFFDE0]/30 text-[#C5F2ED] w-full max-w-md h-full p-6 sm:p-8 shadow-2xl flex flex-col justify-between overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div>
          {/* Header */}
          <div className="flex items-center justify-between pb-4 border-b border-[#C5F2ED]/10 mb-6">
            <h3 className="font-serif-display text-xl font-bold text-[#FFFDE0] flex items-center gap-2">
              <Link2 className="w-5 h-5 text-[#FFFDE0]" />
              <span>Configuração de Links</span>
            </h3>
            <button
              onClick={onClose}
              className="p-1.5 rounded-full hover:bg-[#C5F2ED]/10 text-[#C5F2ED] hover:text-[#FFFDE0]"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="space-y-5">
            {/* Wordmark */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#FFFDE0] mb-1.5">
              ...
              <input
                ...
                className="w-full bg-[#1b3250] border border-[#C5F2ED]/20 rounded-xl px-3.5 py-2.5 text-sm text-[#FAF8F3] focus:outline-none focus:border-[#FFFDE0]"
              />
            </div>

            {/* Checkout Link */}
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#FFFDE0] mb-1.5 flex items-center justify-between">
                <span>Link de Checkout (Hotmart / Kiwify)</span>
                <span className="text-[10px] text-[#FFFDE0]/80 font-mono">CHECKOUT_LINK_AQUI</span>
              </label>
              <input
                type="url"
              ...
              <div className="mt-1 text-right">
                <a 
                  ...
                  className="text-[11px] text-[#C5F2ED] hover:text-[#FFFDE0] inline-flex items-center gap-1"
                >
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#FFFDE0] mb-1.5 flex items-center justify-between">
                ...
                <span className="text-[10px] text-[#FFFDE0]/80 font-mono">WHATSAPP_LINK_AQUI</span>
              </label>
              <input
                type="url"
          ...
          <div className="mt-8 p-4 rounded-2xl bg-[#1b3250] border border-[#C5F2ED]/10">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-[#FFFDE0] flex items-center gap-1.5">
                <Code className="w-4 h-4" />
                <span>Para Alteração Permanente:</span>
              </span>
              <button
                onClick={copyCode}
                className="text-[11px] text-[#C5F2ED] hover:text-[#FFFDE0] flex items-center gap-1 font-medium"
              >
                {copiedCode ? <Check className="w-3 h-3 text-[#FFFDE0]" /> : null}
        ...
        <div className="pt-6 border-t border-[#C5F2ED]/10 mt-6">
          <button
            onClick={onClose}
            className="w-full py-3 rounded-full bg-[#FFFDE0] text-[#284871] font-bold text-sm uppercase tracking-wider hover:bg-[#FFF8C9]"
          >
            Salvar e Fechar
          </button>
        </div>
